...
- **`CopyModToZomboid.bat <ModName> [TargetModsDir]`** — copies a mod into a local Project Zomboid `mods` directory for in-game testing. Defaults to `%ZOMBOID_MODS_DIR%`, then `%USERPROFILE%\Zomboid\mods`. Run this before any in-game verification step in a DevCycle.

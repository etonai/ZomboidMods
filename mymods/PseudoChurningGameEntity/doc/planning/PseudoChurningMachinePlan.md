...
- "Turn On" starts a cycle (target: 10 minutes of game time, revised 2026-09-13 from the originally-stated 15 minutes — see Step 10) with the washer/dryer audio.
...
6. **Liquid removal on cycle end** — removes a whole multiple of 5L at cycle end; milk simply disappears, no fullness yet. **Done (2026-09-13, DevCycle 006).** Verified in-game: only a completed cycle removes milk (manual "Turn Off" does not), a float-imprecision bug that under-removed exact multiples of 5L (10L only removing 5L) was found and fixed, and all test cases (partial, exact-multiple, full, under-threshold) now pass. See `doc/planning/completed/DevCycle006.md`.
7. **Butter generation via inventory** (revised 2026-09-12, DevCycle 004 Phase 4) — at the end of a cycle, for every 5L of milk removed, place 1 stick of butter directly into the Churning Machine's own item inventory (the 20-encumbrance capacity discovered in DevCycle 004). No fullness value, no "Get Butter" menu item, and no debug testing items are needed — the player retrieves butter by opening/looting the object directly. **Done (2026-09-13, DevCycle 007).** Verified in-game: butter is added correctly for every DevCycle 006 test case (partial, exact-multiple, full, under-threshold, manual-stop), retrievable by looting the object normally. First live confirmation that the item-inventory dependency flagged since DevCycle 004 is reliable to use, though *why* it exists remains unconfirmed (still deferred to post-Step-11). See `doc/planning/completed/DevCycle007.md`.
...
10. **Clean up** — cycle time raised from the 1-minute test duration used while implementing Steps 5-7. **Done (2026-09-13, DevCycle 010).** Revised from the originally-stated 15 minutes to 10 minutes, per Ed's direct instruction. Verified in-game: a full cycle now runs 10 minutes before auto-stopping, and DevCycle 006/007's milk-removal and butter-generation behavior on completion is unaffected. See `doc/planning/completed/DevCycle010.md`.

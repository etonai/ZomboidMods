# DevCycle 005: Grits Combined Rice and Oatmeal Ingredients

**Status:** Abandoned
**Start Date:** 2026-06-29
**Target Completion:** Abandoned 2026-06-29
**Focus:** Abandoned attempt to replace the single-template shortcut with a custom grits ingredient set drawn from vanilla rice and oatmeal ingredients.
...
Fix `PseudoGrits` ingredient eligibility so grits can accept ingredients from both vanilla rice and vanilla oatmeal.

DC 4 switched grits evolved recipes to `Template = Rice` to expose savory ingredients like meat. That was useful for confirming the source of the issue, but it still does not match the desired design. Grits should not be forced to choose one vanilla template. They should have their own `Grits` compatibility key populated from the combined vanilla `Rice` and `Oatmeal` ingredient sets.

## Desired Outcome

Players can add ingredients to grits if those ingredients are valid for vanilla rice or vanilla oatmeal.
...
- The grits evolved recipes use a custom template, likely `Template = Grits`.
- The allowed ingredient entries are supplied by mod-owned item override scripts that add `Grits:N` entries to every vanilla item in the combined rice/oatmeal ingredient set.
...
- [ ] ABANDONED: Decide whether any odd rice-compatible items should be excluded from grits.
...
### Phase 3: Add Custom Grits Ingredient Entries

**Status:** Abandoned

- [ ] ABANDONED: Create a mod-owned generated script file for ingredient compatibility, for example `PseudoGritsIngredientOverrides.txt`.
- [ ] ABANDONED: For each combined ingredient, override or extend the item definition to include `Grits:N` in its `EvolvedRecipe` list.
- [ ] ABANDONED: Preserve the item's existing vanilla `EvolvedRecipe` entries when adding `Grits:N`.
- [ ] ABANDONED: Ensure the script load behavior registers those items with evolved recipes whose `Template = Grits`.
- [ ] ABANDONED: Keep this file generated or clearly marked as generated if created mechanically.

**Technical Notes:**
Project Zomboid stores evolved recipe membership on ingredient item definitions, not as a central list on the evolved recipe. This phase creates the custom list by attaching a `Grits` entry to each allowed ingredient item.

### Phase 4: Update Grits Evolved Recipes

**Status:** Abandoned

- [ ] ABANDONED: Change `GritsBowl` evolved recipe from `Template = Rice` to `Template = Grits`.
- [ ] ABANDONED: Change `GritsPan` evolved recipe from `Template = Rice` to `Template = Grits`.
- [ ] ABANDONED: Change `GritsPot` evolved recipe from `Template = Rice` to `Template = Grits`.
...
- [ ] ABANDONED: Confirm all grits evolved recipes use `Template = Grits`.
- [ ] ABANDONED: Confirm generated/override ingredient entries include both oatmeal-derived and rice-derived items.
- [ ] ABANDONED: Confirm representative oatmeal items have `Grits` entries.
- [ ] ABANDONED: Confirm representative rice/meat items have `Grits` entries.
...
- [ ] NOT RUN: Launch Project Zomboid B42 with `PseudoGrits` enabled and confirm no script parse errors.
- [ ] NOT RUN: Confirm bowl, pan, and pot grits still craft.
- [ ] NOT RUN: Confirm a representative oatmeal-style ingredient can be added to grits.
- [ ] NOT RUN: Confirm ground beef can be added to grits.
- [ ] NOT RUN: Confirm at least one other rice/meat ingredient can be added to grits.
...
1. **Should the combined set be a union or an intersection?**
   Proposed decision: union. The user said grits should draw ingredients from both rice and oatmeal, and the current problem is that one-template approaches exclude one side or the other.
...
- The benefit is that grits finally get a true custom ingredient set instead of being forced into one vanilla template.
...
**Reverted Work:**
- Removed PseudoGritsIngredientOverrides.txt.

...
function ISLocationBasedHole:create(x, y, z, north, sprite)
    ...
    -- Use the pre-determined isSaltwater flag (set when context menu was opened)
    -- This ensures we use the clicked square's location, not where player drags cursor
    local isSaltwater = self.isSaltwater or false;
    local holeSprite;
    local holeName;

    if isSaltwater then
        -- Create saltwater well
        holeSprite = "pseudoed_saltwell_01_0";
        holeName = "SaltwaterWell";
    else
        -- Create plain hole
        holeSprite = "pseudoed_saltwell_01_1";
        holeName = "Hole";
    end
...
function ISLocationBasedHole:render(x, y, z, square)
    ...
    -- Use the sprite that was set in new() based on clicked square
...
function ISLocationBasedHole:isValid(square)
    -- If we have locked coordinates, only allow placement at that exact location
    if self.lockedX and self.lockedY then
        if square:getX() ~= self.lockedX or square:getY() ~= self.lockedY then
            return false;
        end
    end
